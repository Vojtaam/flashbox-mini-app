// FlashBOX App - Kompletně opravená verze 
// Zachována všechna funkcionalita při opravě kritických problémů
...
class FlashBoxApp {
    ...
    checkTelegramAPI() {
        if (typeof window.Telegram !== 'undefined' && window.Telegram.WebApp) {
            try {
                ...
                // TELEGRAM WEBAPP INICIALIZACE - OPRAVENO
                this.telegram.ready();
    ...
    setupEventListeners() {
        // OPRAVENO: Správné ID selektory
        const state1Btn = document.getElementById('state1-btn');
        const state2Btn = document.getElementById('state2-btn');
        
        if (state1Btn) {
            state1Btn.addEventListener('click', (e) => this.selectState(1, e));
        }
        
        if (state2Btn) {
            state2Btn.addEventListener('click', (e) => this.selectState(2, e));
        }
    ...
    generateMessage(stateId, description) {
        ...
        // Formát zprávy pro n8n automatizaci - OPRAVENO
